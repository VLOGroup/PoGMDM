...
class BlockAdam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, noise=None, amsgrad=False):
        if not 0.0 <= lr:
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, noise=noise, amsgrad=amsgrad)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                if noise and p.numel() > 1:
                    # add noise
                    if hasattr(p, 'reduction_dim'):
                        norm = torch.sum(p.data**2, p.reduction_dim, True).sqrt_()
                    else:
                        norm = p.data.abs()
                    p.data.add_(torch.randn_like(p.data)*norm*noise)
